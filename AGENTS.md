...
- Billing and the AI relay are split across `lib/apikeys.ts` (key/card-code generation), `lib/quota.ts` (quota state, token estimation, `cost = max(1, ceil(promptTokens × inputRate + completionTokens × outputRate))`), `lib/ai-proxy.ts` (Bearer-key auth + OpenAI error bodies), and the `/v1/*` routes (upstream failover by `priority`, streaming SSE passthrough with usage sniffing, post-completion quota settlement). Rates are per-upstream: `upstreams.modelRates[modelId]` overrides the `ai_models` catalog defaults, which are derived from models.dev pricing (api.json `cost` fields) normalized against `deepseek/deepseek-v4.1-flash` as the 1.0 baseline (`lib/model-metadata.ts`, 24h cache; legacy single `rate` remains a read-time fallback). Card codes embed a creation-time plan snapshot so plans can be edited or deleted without invalidating outstanding cards.
